<?php
// Muhammad Adam Canrayneldy: sabtu, Maret 2020
/*
BOT PENGANTAR
...
// canray: spesifik pesan(seperti menggunakan :id)
...
{
    global $debug;
    // canray: untuk parameter pada url
    $data = array(
        'chat_id' => $chatid,
        'text'  => $text,
    ...
    $options = array(
        'http' => array(
            ...
            'method'  => 'POST',
            // canray: membuild array(aasoc atau index) menjadi parameter url. 
            // contoh output -> "chat_id=123&text=halo&reply_to_message_id=321"
            'content' => http_build_query($data),
        ),
    );
    // canray: stream_context_create($['wrapper']['optionForHttp']) --> membuat stream context
    $context  = stream_context_create($options);
    // canray: file_get_contents($pathUrl,$include_path,$context) --> membaca seluruh file menjadi string 
    $result = file_get_contents(request_url('sendMessage'), false, $context);

    // canray: tampilkan debbugging
    if ($debug) 
        print_r($result);
}
...
function process_one()
{
    ...
    // canray: ambil update_id terakhirnya
